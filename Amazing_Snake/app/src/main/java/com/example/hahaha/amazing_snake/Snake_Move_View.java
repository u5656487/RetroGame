...
public class Snake_Move_View extends  View  implements Runnable, View.OnTouchListener {
    Handler timer;
    Random random;
    Snake_Body snake;
    ...
    public Snake_Move_View(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        ...
        fruit=new Fruits(new Point( (float)(2.5*width),(float) (half_gap+1.5*width)),FruitType.Apple );
    ...
    // move the snake, check whether there is crash, and whether the snake can eat fruits.
    @Override
    public void run() {
        snake.snake_move();
        check_crash();
...
//check whether the snake was hit the wall, or hit itself. If hitted, initialize a new snake.
